...
def rec_not_format_string(node):
    if node.type == 'string':
        if node.text.decode('utf-8')[0] != 'f' and len(node.parent.children) > 2 and node.parent.children[2].text != b'format':
            return True

def rec_format_string_left(node):
    if node.type == 'string':
        if node.text.decode('utf-8')[0] == 'f':
            return True
...
def cvt_single_quotation(node):
    # ''
    if rec_string(node):
        str = node.text.decode('utf-8').strip()
...
def cvt_LeftF2RightFormat(node):
    # f'{a}{b}' -> '{}{}'.format(a, b)
    if rec_format_string_left(node):
        str = node.text.decode('utf-8').strip()
...
def cvt_RightFormat2LeftF(node):
    # '{}{}'.format(a, b) -> f'{a}{b}'
    if rec_format_string_right(node):
        str = node.text.decode('utf-8').strip()
        params_node = node.parent.parent.children[1]
        params_list = params_node.text.decode('utf-8')    # (a, b)
