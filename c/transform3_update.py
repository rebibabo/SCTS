...
def cvt_ToAugment(node):
    # i += 1
    if rec_LeftUpdate(node) and 'expression' not in node.parent.type:   # 不是a[++i] 不是*p(++i)不是a=++i
        op = text(node.children[0])[0]
        param = text(node.children[1])
        return [(node.end_byte, node.start_byte), 
                (node.start_byte, f"{param} {op}= 1")]
    if rec_RightUpdate(node) and 'expression' not in node.parent.type:
        op = text(node.children[1])[0]
...
def cvt_ToAssignment(node):
    # i = i + 1
    if rec_LeftUpdate(node) and 'expression' not in node.parent.type:
        op = text(node.children[0])[0]
        param = text(node.children[1])
        return [(node.end_byte, node.start_byte), 
