...
def cvt_for(node, code):
    ...
    if prev.type == 'local_variable_declaration' and prev.child_count == 3 or prev.type == 'expression_statement' and prev.children[0].type in ['update_expression', 'assignment_expression']:
...
def cvt_while(node, code):
    ...
    abc = get_for_info(node)
    for child in node.children:
        if child.type == 'block' or not child.next_sibling:
            block_node = child
    ret = [(block_node.start_byte, node.start_byte)]    # 删除for(a;b;c)
    if abc[0] is not None:  # 如果有a
        indent = get_indent(node.start_byte, code)
        if abc[0].type != 'local_variable_declaration':
            ret.append((node.start_byte, text(abc[0]) + f';\n{indent * " "}'))
